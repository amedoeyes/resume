...
export default function Input({ label, className = "", ...rest }: InputProps) {
	return (
		<div className={`relative ${className}`}>
			<input
				className="peer w-full appearance-none rounded-full border-2 border-primary border-opacity-25 bg-secondary p-2 outline-none transition-[border-color] duration-200 hover:border-opacity-50 focus:border-opacity-100"
				placeholder=" "
				{...rest}
			/>
			<label
